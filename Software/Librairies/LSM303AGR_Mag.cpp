...
void LSM303AGR_Mag::init(uint8_t AODR)
{
  uint8_t AODR_mag;
 ...
 _i2c_bus->writeByte(LSM303AGR_Mag_ADDRESS, LSM303AGR_MAG_CFG_REG_A, 0x80 | 0x02<<2);  // high-resolution mode
 _i2c_bus->writeByte(LSM303AGR_Mag_ADDRESS, LSM303AGR_MAG_CFG_REG_A, 0x80 | 0x02<<2 | 0x10);  // low power mode

 // enable low pass filter (bit 0 == 1), set to ODR/4, enable offset-cancellation bit 1
 _i2c_bus->writeByte(LSM303AGR_Mag_ADDRESS, LSM303AGR_MAG_CFG_REG_B, 0x01);  //0x02
...
void LSM303AGR_Mag::readMagData(int16_t * destination)
{
  ...
  destination[0] = ((int16_t)rawData[1] << 8) | rawData[0];  // Turn the MSB and LSB into a signed 12-bit value in normal mode
